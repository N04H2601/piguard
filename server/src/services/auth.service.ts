...
import { apiKeysRepo, revokedTokensRepo, settingsRepo } from '../database/repositories.js';
...
const JWT_ISSUER = 'piguard';
const JWT_AUDIENCE = 'piguard-session';
...
function resolveJwtSecret(): string {
  ...
  if (config.JWT_SECRET && config.JWT_SECRET.length >= 32) {
    settingsRepo.set(JWT_SECRET_KEY, config.JWT_SECRET);
...
export async function verifyToken(token: string): Promise<SessionPayload | null> {
  try {
    ...
    if (revokedTokensRepo.isRevoked(payload.jti)) {
      return null;
    }
    return {
      sub: payload.sub,
...
export async function revokeToken(token: string): Promise<void> {
  try {
    ...
    revokedTokensRepo.revoke(payload.jti, payload.exp * 1000);
  } catch {
