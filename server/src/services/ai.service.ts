...
import { getAiSettings, getAppLanguage } from './setup.service.js';
...
export function getConversationWelcomeMessage() {
  ...
  if (language === 'en') {
    return 'Hello. Ask me about the dashboard, alerts, services, or security events. I will answer briefly and operationally.';
  }
  return 'Bonjour. Pose-moi une question sur le dashboard, les alertes, les services ou les evenements de securite. Je repondrai de facon breve et operationnelle.';
}
...
export async function createDashboardAdvice(messages: AssistantMessage[]) {
  ...
  const modelsToTry = [...new Set([config.openaiModel, 'gpt-5.4', 'gpt-5'])];
