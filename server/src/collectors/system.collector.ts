...
export function collectCpu() {
  ...
  const loadAvg = readFileSync('/proc/loadavg', 'utf-8').split(/\s+/).slice(0, 3).map(Number);
...
export function collectUptime() {
  const content = readFileSync('/proc/uptime', 'utf-8').trim().split(/\s+/);
  return {
...
export async function collectAll() {
  const [temp, disk, processes] = await Promise.all([
    collectTemperature(),
    collectDisk(),
    collectProcesses(),
  ]);
