...
export async function collectWireGuard() {
  ...
  try {
    ...
    for (const line of lines) {
      ...
      const isLegacyInterfaceLine = parts.length === 4 && /^\d+$/.test(parts[2] ?? '');
      const isDumpInterfaceLine = parts.length >= 5 && /^\d+$/.test(parts[3] ?? '');

      if (isLegacyInterfaceLine || isDumpInterfaceLine) {
        // `wg show all dump` interface rows typically include:
        // iface, privateKey, publicKey, listenPort, fwmark
        // Older/alternate formats may omit the private key column.
        const name = parts[0]!;
        if (!validIface.test(name)) continue;

        const publicKey = isDumpInterfaceLine ? (parts[2] ?? '') : (parts[1] ?? '');
        const listenPort = parseInt((isDumpInterfaceLine ? parts[3] : parts[2]) ?? '0', 10);
