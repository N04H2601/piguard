...
export class SecurityPanel extends LitElement {
    ...
    return html`
      ...
      <div class="tabs">
        ${(['overview', 'fail2ban', 'auth', 'logins'] as const).map((tabName) => html`
          <button class="tab ${this.tab === tabName ? 'active' : ''}" @click=${() => this.tab = tabName}>${this.getTabLabel(tabName)}</button>
        `)}
      </div>
  ...
  private renderAuth() {
    if (this.authEvents.length === 0) {
      return html`<pg-empty-state title="No auth events" detail="No recent SSH or sudo entries were matched from host logs or journald."></pg-empty-state>`;
    }
        ...
        <table class="table">
          ...
          <tbody>
            ${this.authEvents.slice(0, 100).map((event) => html`
              <tr>
                <td>${formatLocalDateTime(event.timestamp, event.timestamp)}</td>
  ...
  private renderLogins() {
    if (this.loginHistory.length === 0) {
      return html`<pg-empty-state title="No dashboard login history" detail="Only web dashboard login attempts are listed here. Existing sessions, API keys, and SSH logins are tracked elsewhere."></pg-empty-state>`;
    }
        ...
        <table class="table">
          ...
          <tbody>
            ${this.loginHistory.map((login: any) => html`
              <tr>
                <td>${formatLocalDateTime(login.timestamp)}</td>
                <td>${login.ip}</td>
  ...
  private getTabLabel(tabName: 'overview' | 'fail2ban' | 'auth' | 'logins') {
    switch (tabName) {
      case 'auth':
        return 'SSH / Sudo';
      case 'logins':
